...
> ⚠️ **`POST` 成功、拿到稿號＝草稿建好了，不等於稿子做完。** 2026-08-18 曾發生 `POST` 建完稿
> 直接回報「完成」、漏掉延伸閱讀（要求至少10篇）的情況——延伸閱讀在文件裡本來就有寫（見下方
> 「延伸閱讀」章節＋「發稿檢查清單」），漏掉的原因是沒把清單當成建稿流程的強制最後一步。
> **建稿收尾前一定要走一次「發稿檢查清單」（見下方章節），逐項confirm，不要只看`POST`回200就結案。**
...
### ⚠️ 用 `file_upload` 上傳後別再手動點完彈窗（2026-08-17 補充）

`file_upload` 只是拿來解決「本機檔案路徑 → 瀏覽器可用 bytes」這一步（見下方「純 API 上傳」
章節：這一步真的沒辦法完全純 API，因為需要一個現成的 `fileBlob`，而這個 Blob 目前只有
真實 file input 的選檔事件能自然產生）。**選檔完成、S3 自動上傳那一刻起，後面全部能純 API
做完**，不需要照著「上傳圖片」彈窗手動點「來源→裁切比例→完成」：

1. `file_upload` 塞路徑進 file input（觸發自動 S3 上傳，拿到 `uuid`）
2. 直接呼叫 `GET /api/v1/images/process?uuid=...&width=...&height=...&crop_aspect_ratio_type=...`（自己組參數，不用等彈窗）
3. 把結果組進 `asset_images` 直接 PUT 文章

2026-08-17 實測踩的坑：對 4 張圖都乖乖照著彈窗手動點「16:9/4:3」「選來源」「完成」，
等於把本來能純 API 做的事又走了一次 UI，多花了不必要的時間。下次應該選完檔後**立刻**
用 JS 攔截 S3 上傳請求拿 `uuid`（或直接呼叫 `upload-url` API 自己拿，見下方章節），
略過彈窗的手動操作。
...
## ⚠️「建立新文章」頁面的儲存按鈕壞掉（2026-08-17 拓測確認）

`/articles/create` 頁面點「儲存」**永遠無法成功**，會固定打：

```
PUT /api/v1/articles/NaN → 422 {"error_code":"FIELD_VALIDATION_FAILED","error_message":"文章 ID 必須是整數","meta":{"field":"id"}}
```

**重現步驟**（2026-08-17 用乾淨的新分頁重測過，非狀態污染，每次都一樣）：
1. 開 `/articles/create`，把所有前端必填欄位（長標題/短標題/來源/分類/內文/標籤）填到通過
   前端驗證（紅字「不得為空」消失、`請檢查所有欄位是否填寫正確` toast 不再跳出）
2. 攔截 `XMLHttpRequest.prototype.open/send`（這個後台用 axios 走 XHR，`fetch` 攔不到）
3. 點「儲存」→ 一律打 `PUT /articles/NaN`，422

**原因推測**：新建／編輯應該是共用同一個表單元件，但「新建」模式下缺了「先 POST 拿到
真實 id、再切換成 PUT」這一步，直接把還不存在的 id（undefined）塞進 URL 變成 `NaN`。

**唯一可行解法**：`/articles/create` 頁面**不要靠 UI 的儲存按鈕**，直接呼叫
`POST /api/v1/articles`（見上方「建立文章」章節），成功後拿到真實 `id`。
之後要補議題包/延伸閱讀/圖片等欄位，改導到 `/articles/{id}`（**這個編輯頁的儲存按鈕是正常的**，
同樣攔截驗證過：點下去正確打 `PUT /articles/{id}` → 200），或直接用 `PUT /api/v1/articles/{id}`
純 API 補欄位都可以。

**教訓**：以後任何一次「點儲存」前，先用 XHR/fetch 攔截確認實際打出的 method 是不是
自己預期的那個（尤其是新建頁），不要假設 UI 儲存按鈕一定跟文件早期拓測時行為一致——
這代表後台前端在拓測之後可能又改版，儲存邏輯已經跟 2026-08-05 記錄的不一樣了。

## ⚠️ 內文編輯器「請輸入內文...」placeholder 殘留 bug（2026-08-17 踩雷）

在**真正空白**的內文編輯器（`內文 (0)`）點擊聚焦後直接打字，placeholder 文字
「請輸入內文...」有機率沒有被正常清除，變成跟使用者輸入的文字黏在一起存進內容
（例如打「烏克蘭發動...」結果存成「請輸入內文...烏克蘭發動...」）。可重現，發生兩次。

**解法**：點進編輯器後，**先 Ctrl+A + Delete 清空一次**（此時編輯器裡只有 placeholder，
沒有真實內容，Ctrl+A 在這個情境下是安全的），確認 `內文 (0)` 之後再開始打字。

**反例（真正危險的情況）**：編輯器裡**已經有真實內文**時，千萬別在圖說欄位或任何子輸入框
按 Ctrl+A 想清空，一旦焦點沒有真的落在子輸入框而是落回主編輯器，Ctrl+A 會選到**整篇內文**，
接下來的輸入會直接覆蓋掉全部內容（2026-08-17 實際發生過一次，用 Ctrl+Z 挽回）。
子輸入框（圖說、標籤等）要清空重打，一律用 `Home` → `Shift+End` 選取整行再輸入，不要用 Ctrl+A。
